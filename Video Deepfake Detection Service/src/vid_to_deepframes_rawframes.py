...
def vid_to_frames_rawframes(video):
    # image_name_video = []
    image_path = 'Video Deepfake Detection Service/videos'
    # Load the cascade
    face_cascade = cv2.CascadeClassifier(os.path.join('Video Deepfake Detection Service/src','haarcascade_frontalface_default.xml'))
    ...
    for f in [f for f in os.listdir(image_path)]:
        ...
        ruta_parcial = os.path.join('Video Deepfake Detection Service/videos/DeepFrames/',f)
        if not(os.path.exists(ruta_parcial)) :
            os.mkdir(ruta_parcial);
        ruta_parcial2 = os.path.join('Video Deepfake Detection Service/videos/RawFrames/',f)
